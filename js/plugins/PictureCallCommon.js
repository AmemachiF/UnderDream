 ...
 * @text 图片编号的变量
 * @desc 存储最后发生事件的图片编号的变量ID。
 * @default 0
 * @type variable
 *
 * @param InvalidSwitchId
 * @text 无效开关
 * @desc 指定编号的开关为ON时，所有图片事件都无效。
 * @default 0
 * @type switch
 * 
 * @param FlickDistance
 ...
 * @text 浮动时间变量
 * @desc 存储轻拂判定生效前所花的帧数的变量ID。这个值越小越快。
 * @default 0
 * @type variable
 *
 * @param FlickDegreeVariable
 * @text 浮动角度变量
 * @desc 是用度数法存储轻拂判定生效时的轻拂方向的变量ID。
 * @default 0
 * @type variable
 *
 ...
 * @option 4:鼠标与图片重叠时
 * @value 4
 * @option 5:当鼠标与图片重叠时将鼠标从图片中释放时
 * @value 5
 * @option 6:释放点击时
 * @value 6
 * @option 7:单击时（且考虑长按时的重复）
 * @value 7
 * @option 8:点击时一直
 * @value 8
 * @option 9:点击滚轮时（仅在PC时有效）
 * @value 9
 * @option 10:双击时
 * @value 10
 * @option 11:鼠标在图片中移动时
 * @value 11
 * @option 12:按住鼠标在图片内移动时
 * @value 12
 * @option 13:轻拂（鼠标按压状态下移动指定距离以上）时
 * @value 13
 *
 * @arg invalidSwitchId
 * @text 无效开关
 * @desc 指定编号的开关为ON时，图片事件无效。
 * @default 0
 * @type switch
 *
 ...
 *
 * 对图片的各种鼠标操作进行反应，可以执行共模事件和控制开关。
 * （这被称为图片事件）注册过一次的图片事件即使删除图片也有效。
 * 可通过专用插件命令解除。
 *
 * 图片事件不管是地图画面还是战斗画面，都不受其他事件和状况的限制，
 * 迅速并行执行。另外，在画面事件发生瞬间的帧中，其他的点击处理无效，
 * 不通过触摸进行地图移动或发送信息等。
 *
 * 使用条款：
 * 作者可以擅自更改、再发布，使用形式（商务、18禁使用等）
 * 也没有限制。
 * 这个插件已经是你的了。
 */
